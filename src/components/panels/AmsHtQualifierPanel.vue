            ...
            <v-tabs-items v-model="tab">
                <v-tab-item>
                    ...
                    <div class="d-flex flex-wrap">
                        <v-btn color="error" class="mr-2 mb-2" :disabled="!canCommand" @click="startDryer">
                            {{ $t('Panels.AmsHtQualifierPanel.StartDryer') }}
                        </v-btn>
                        <v-btn class="mr-2 mb-2" :disabled="!canCommand" @click="sendGcode('AMS_HT_DRYER_STOP')">
                            {{ $t('Panels.AmsHtQualifierPanel.StopCooldown') }}
                        </v-btn>
                        <v-btn class="mb-2" :disabled="!canCommand" @click="sendGcode('AMS_HT_CLEAR_FAULT')">
...
import { mdiRefresh, mdiTuneVariant } from '@mdi/js'
import BaseMixin from '@/components/mixins/base'
import { buildAmsHtQualifierModel, AmsHtQualifierModel } from '@/components/panels/AmsHtQualifier/adapter'
...
export default class AmsHtQualifierPanel extends Mixins(BaseMixin) {
    ...
    get model(): AmsHtQualifierModel {
        return buildAmsHtQualifierModel(this.$store.state.printer)
    }
    ...
    get hasFault(): boolean {
        const fault = this.model.dryer?.fault
        return Boolean(fault && fault !== 'none')
    }
    ...
    sendGcode(command: string): void {
        this.$store.dispatch('printer/sendGcode', command)
    }
